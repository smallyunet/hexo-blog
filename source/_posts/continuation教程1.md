...
可以看到，对于需要两次递归调用的情况，CPS 是把另一次递归调用，写在了原本的 `r => k(r)` 函数里，让第二次内部调用成为了递归调用 `fib` 时候的子调用。这句话有点绕，主要是结合代码理解一下。
...
我们以计算 3 的斐波那契数为例，拆解一下具体的执行步骤。要注意的是，这个过程非常复杂，比 `fact` 函数还要复杂很多：
