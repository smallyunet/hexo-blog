...
[猜均值的2/3](https://zh.m.wikipedia.org/zh-cn/%E7%8C%9C%E5%9D%87%E5%80%BC%E7%9A%842/3) 是一个经典的博弈游戏，我现在想要把它改造成运行在区块链上的 dApp 游戏，这是游戏的基本规则：
...
5. 如果多个用户支付的 `任意金额` 相同，第一个参与的用户获胜
6. 获胜者获得本轮游戏所有用户支付的 `金额总和`
7. 合约在每轮游戏抽取 `金额总和` 的 1% 作为服务费

一个重要的问题是，每轮游戏多少玩家参与？或者说每轮游戏什么时候结束？关于游戏结束的规则是这样：

8. 每一轮游戏，当玩家参与的时候，可以知道玩家是第 `user` 个参与的
9. 生成一个随机数 `n`（10<=n<=100），
10. 根据 `user` 和 `n`，生成一个范围 `l, r = user-n, user+n`
11. 在 `l, r` 的范围内生成一个随机数 `x`
12. 如果 `x == user`，本轮游戏结束

上面是这个游戏的全部规则。

结束游戏的规则有点复杂，举一个具体的数值，比如有一个玩家，是第 `user=50` 个参与进游戏的，然后生成了一个随机数 `n=10`，那么生成的范围就是 `l, r = 45, 55`。在范围 [45, 55] 中再生成一个随机数，如果正好是 50，本轮游戏结束。

为什么要这样做？因为区块链上的数据是公开的。在原版的“猜均值的2/3”游戏中，玩家选择的数字，相互是保密的。但在区块链上很难做到这一点。所以这里提出的 dApp 版本游戏规则，至少有两点和原版规则不同：

- 玩家参与游戏支付的金额是无限制的，可以是 0，也可以无限大。原版规则有上限
...
然后解释一下游戏结束的规则。这个规则的效果是，能在大多数情况下，让游戏在参与人数为 [10, 100] 的区间内结束。这个区间对应 `规则 9` n 的范围。

这个规则可以这样理解，每个用户都拥有能力结束游戏，但是能力是随机的，最高 1/10，最低 1/100。拥有 1/10 的能力，就是有 1/10 的可能性让游戏结束。
...
所以 `规则 9` 中 n 的范围大概就限定了参与人数的范围。当然，这里的人数范围 [10, 100] 不是严格的概率计算。

我按照上面定义的游戏结束规则，模拟了 100 万次运行的结果，记录下游戏会在“有多少个玩家参与”的时候结束，代码在这里：[guessavg/emulate_tool](https://github.com/guessavg/emulate_tool)

<img src="chart1.png" style="width:60%">

这个图的横坐标是参与游戏的玩家数量，纵坐标是在对应的数量上，游戏结束的次数。比如在接近 1 的位置上，有 2500 次，说明在 100 万次游戏中，有 2500 次 1 个玩家参与就结束了。
